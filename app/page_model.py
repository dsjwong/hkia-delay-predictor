...
def render() -> None:
    ...
    st.markdown("#### Live evaluation — predictions vs what happened")
    ev = D.live_eval(7)
    if ev["status"] != "ok":
        st.info(f"Collecting — **{ev['n_matured']}** matured predictions so far (need ≥ {ev['min_n']}). A prediction matures when its flight "
                "departs; the metric uses the last score written before departure. Scoring started 2026-08-17.")
    else:
