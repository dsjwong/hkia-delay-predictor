...
export function Sparkline({ history, width = 260, height = 56 }: { history: [number, number | null, number | null][]; width?: number; height?: number }) {
  ...
  const y = (p: number) => 4 + (1 - p) * (height - 12)
  const d = pts.map((p, i) => `${i ? 'L' : 'M'}${x(p[0]).toFixed(1)},${y(p[1]).toFixed(1)}`).join(' ')
  return (
    <svg width={width} height={height} role="img" aria-label={`P(delay > 15) over ${pts.length} scores from ${hm(pts[0][0])} to ${hm(t1)} HKT`} className="block max-w-full">
      <line x1={4} x2={width - 4} y1={y(0.5)} y2={y(0.5)} stroke={GRID} strokeWidth={1} />
      <path d={d} fill="none" stroke={BLUE} strokeWidth={1.5} />
      {pts.map((p, i) => (
        <circle key={i} cx={x(p[0])} cy={y(p[1])} r={i === pts.length - 1 ? 3.2 : 2} fill={BLUE} stroke="#0b1220" strokeWidth={1}>
          <title>
