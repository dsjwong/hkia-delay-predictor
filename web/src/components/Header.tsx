  ...
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-surface/90 backdrop-blur">
      <div className="mx-auto flex max-w-[1600px] items-center gap-3 px-3 sm:px-4 h-12">
        <NavLink to="/" className="flex items-center gap-2 text-ink no-underline shrink-0" aria-label="HKIA delay predictor — home">
          <svg width="22" height="22" viewBox="0 0 64 64" aria-hidden="true">
            <rect width="64" height="64" rx="12" fill="#121c2e" />
            <path fill="#ffbf3d" d="M32 6c2.4 0 4 3.6 4 8v14l24 14v6l-24-7v13l6 5v4l-10-3-10 3v-4l6-5V41L8 48v-6l24-14V14c0-4.4 1.6-8 4-8z" />
          </svg>
          <span className="font-semibold tracking-[0.01em]">HKIA departures</span>
          <span className="hidden md:inline text-[0.7rem] uppercase tracking-[0.08em] text-muted ml-1">delay predictor · VHHH</span>
        </NavLink>
        <nav className="hidden md:flex items-center gap-1 ml-4" aria-label="Pages">
          {NAV.map((n) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn('px-2.5 py-1 rounded-md text-sm no-underline', isActive ? 'bg-surface-3 text-ink' : 'text-ink-2 hover:text-ink hover:bg-surface-2')
              }
            >
              {n.label}
            </NavLink>
        ...
        <div className="ml-auto flex items-center gap-3">
          <Tooltip
            ...
            side="bottom"
          >
            <span className="flex items-center gap-2 font-mono text-[0.74rem] text-ink-2 whitespace-nowrap" tabIndex={0}>
          ...
          <a href={REPO} className="hidden sm:inline-flex text-ink-2 hover:text-ink" aria-label="Source on GitHub" title="Source on GitHub">
          ...
          <button className="md:hidden text-ink-2 hover:text-ink" aria-label="Menu" aria-expanded={open} onClick={() => setOpen((o) => !o)}>
        ...
        <nav className="md:hidden border-t border-border bg-surface-2 px-3 py-2 flex flex-col" aria-label="Pages">
          {NAV.map((n) => (
            <NavLink
              ...
              className={({ isActive }) => cn('px-2 py-2 rounded-md text-sm no-underline', isActive ? 'bg-surface-3 text-ink' : 'text-ink-2')}
            >
              {n.label}
            </NavLink>
          ))}
