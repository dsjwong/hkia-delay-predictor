...
export function PBar({ p, className, width = 64 }: { p: number | null | undefined; className?: string; width?: number }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-2 hk-num', className)} title={`P(delay > 15 min) = ${pct(p)}`}>
      <span className="h-[6px] rounded-full bg-surface-3 overflow-hidden" style={{ width }} aria-hidden="true">
        <span className="block h-full rounded-full" style={{ width: `${Math.round(p * 100)}%`, background: amberHex(p) }} />
      </span>
