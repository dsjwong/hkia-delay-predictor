...
export function Timeline({ flights, onPick, names }: { flights: Flight[]; onPick?: (f: Flight) => void; names: (code: string | null) => string }) {
  const pts = (st: 'departed' | 'scheduled'): Pt[] =>
    flights.filter((f) => f.p != null && f.status === st).map((f) => ({ x: minuteOfDayHKT(f.sched_ts), y: f.p as number, f }))
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <ScatterChart margin={{ top: 8, right: 12, bottom: 4, left: -12 }}>
          ...
          <XAxis type="number" dataKey="x" domain={[0, 1440]} ticks={[0, 180, 360, 540, 720, 900, 1080, 1260, 1440]} tickFormatter={hhmm} {...AXIS} name="scheduled (HKT)" />
          <YAxis type="number" dataKey="y" domain={[0, 1]} tickFormatter={(v) => `${Math.round(v * 100)}%`} {...AXIS} width={44} />
              ...
              return (
                <TipBox
                  ...
                  rows={[
                    ...
                    ...(f.delay_min != null ? ([['actual delay', `${signed(f.delay_min)} min`]] as [string, string][]) : []),
                  ]}
                />
              )
            }}
          />
          <Scatter name="departed" data={dep} fill={BLUE} stroke="#0b1220" strokeWidth={1} r={4} onClick={(d) => onPick?.((d as unknown as Pt).f)} cursor="pointer" isAnimationActive={false} />
          <Scatter name="not yet departed" data={sch} fill={ORANGE} stroke="#0b1220" strokeWidth={1} r={4} onClick={(d) => onPick?.((d as unknown as Pt).f)} cursor="pointer" isAnimationActive={false} />
        </ScatterChart>
      </ResponsiveContainer>
    </div>
  )
...
export function HourlyBars({ rows }: { rows: HourRow[] }) {
  return (
    <div>
      ...
      />
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={rows} margin={{ top: 8, right: 8, bottom: 4, left: -12 }} barCategoryGap="30%" barGap={2}>
          <CartesianGrid stroke={GRID} vertical={false} />
          <XAxis dataKey="hour" {...AXIS} tickFormatter={(h) => String(h).padStart(2, '0')} interval={1} />
          <YAxis domain={[0, 1]} tickFormatter={(v) => `${Math.round(v * 100)}%`} {...AXIS} width={44} />
          <Tooltip
